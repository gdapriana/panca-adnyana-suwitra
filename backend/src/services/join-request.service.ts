...
class JoinRequestService {
  ...
  static async create(username: string, stt_slug: string) {
    ...
    const joinCheck = await prismaClient.join_request.findFirst({
      where: {
        AND: [{ username: username }, { stt_slug: stt_slug }],
      },
    });
    ...
    if (userCheck.stt_membership)
      throw new ResponseError(401, "sudah menjadi anggota");
  ...
  static async acc(req_id: string, admin_stt: string) {
    ...
    if (joinRequest.stt_slug !== admin_stt)
      throw new ResponseError(401, "tidak diizinkan");
    if (joinRequest.user.stt_membership)
      throw new ResponseError(401, "sudah menjadi anggota");
  ...
  static async delete(id: string, req: UserRequest) {
    ...
    if (req.user!.role !== "ADMIN")
      throw new ResponseError(401, "tidak diizinkan");
    if (!joinRequest) throw new ResponseError(404, "request tidak ditemukan");
    if (req.user?.stt_membership!.stt_slug !== joinRequest.stt_slug)
      throw new ResponseError(401, "tidak diizinkan");
