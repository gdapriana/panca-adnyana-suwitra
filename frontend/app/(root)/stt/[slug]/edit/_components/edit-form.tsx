...
export default function SttEditForm({
	...
	const [defaultEmail, setDefaultEmail] = useState<string | undefined>(stt?.email);
	const [defaultDescription, setDefaultDescription] = useState<string | undefined>(stt?.description);
	const [defaultIGUrl, setDefaultIGUrl] = useState<string | undefined>(stt?.instagram_url);
	const [defaultWAUrl, setDefaultWAUrl] = useState<string | undefined>(stt?.whatsapp_url);
	const [defaultFBUrl, setDefaultFBUrl] = useState<string | undefined>(stt?.facebook_url);
	const [defaultLogoUrl, setDefaultLogoUrl] = useState<string | undefined>(stt?.logo_url);
	const [defaultBackgroundUrl, setDefaultBackgroundUrl] = useState<string | undefined>(stt?.background_url);
	...
	const handleSubmit = async (e: any) => {
		...
		await fetchPatchSTT({
			...
			sttProps: {
				...
				description: defaultDescription === stt?.description ? undefined : defaultDescription,
				instagram_url: defaultIGUrl === stt?.instagram_url ? undefined : defaultIGUrl,
				facebook_url: defaultFBUrl === stt?.facebook_url ? undefined : defaultFBUrl,
				whatsapp_url: defaultWAUrl === stt?.whatsapp_url ? undefined : defaultWAUrl,
	...
	return (
		<form onSubmit={handleSubmit} className="grid gap-6 grid-cols-1 md:grid-cols-2 md:grid-rows-5">
			<InputText
				name="name"
