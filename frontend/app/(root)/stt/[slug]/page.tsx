...
export default async function SttPage({ params }: { params: { slug: string } }) {
		...
		<main className="flex justify-center py-4 px-4 items-center">
			<div className="w-full max-w-6xl gap-8 flex justify-center flex-col items-stretch">
				<div className="flex flex-col justify-start items-stretch">
					<div className="relative rounded-3xl flex justify-center items-center h-[300px] mb-[3rem] md:mb-[4rem] md:aspect-[16/4] bg-secondary">
						{stt.background_url ? (
							<Image
						...
						<div className="absolute overflow-hidden flex justify-center items-center bottom-[-15%] left-1/2 right-1/2 translate-x-[-50%] w-[80px] md:w-[100px] aspect-square bg-white rounded-full">
							{stt.logo_url ? (
								<Image src={stt.logo_url} alt="logo" width={1000} height={1000} className="w-full h-full object-cover" />
							) : (
								<span className="text-muted-foreground">
									<ImageOff />
								</span>
					...
					<div className="flex gap-4 justify-center items-center flex-col">
						<h1 className="font-bold text-xl md:text-3xl text-center">STT {stt.name}</h1>
						<SttInformation stt={stt} />
						<p className="text-muted-foreground text-center">{stt.description}</p>
						<UserActions stt={stt} />
					</div>
				</div>
