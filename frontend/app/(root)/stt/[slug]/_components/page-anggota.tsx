...
export default function PageAnggota({ memberships, stt_slug }: { memberships: SttMembership[]; stt_slug: string }) {
				...
				<Table>
					<TableHeader>
						<TableRow>
							...
							{authenticated && (role === "ADMIN" || role === "SUPERADMIN") && <TableHead>Aksi</TableHead>}
						</TableRow>
					</TableHeader>
					<TableBody>
							...
							return (
								<TableRow key={index}>
									...
									<TableCell>
										...
										{member.role === "MEMBER" && <Badge variant="secondary"> Anggota</Badge>}
									</TableCell>
									<TableCell>{moment(member.join_date).format("DD MMM YYYY")}</TableCell>
