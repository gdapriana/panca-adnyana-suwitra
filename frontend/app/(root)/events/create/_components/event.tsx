...
export default function EventForm({
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/${role === "USER" ? "sttevent" : "mainsttevent"}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    ...
    >
      ...
      <Label
        htmlFor="description"
        className="flex md:col-span-3 flex-col justify-start items-start"
      >
      ...
      <Button
        disabled={buttonDisabled || !validateDates()}
        type="submit"
        className=""
      >
        Buat event
