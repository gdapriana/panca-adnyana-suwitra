...
export const fetchSTT = async ({
	...
	try {
		setLoading(true);
		const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/stt/${slug}`);
...
export const fetchPatchSTT = async ({
	...
	let cloudResponseLogo: { url: string; public_id: string } | undefined = undefined;
	let cloudResponseBackground: { url: string; public_id: string } | undefined = undefined;
	try {
		setLoading(true);
		...
		const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/${user_role === "ADMIN" ? "stt" : "mainstt"}/${slug}`, {
			method: "PATCH",
			headers: {
			...
			body: JSON.stringify({
				...
				background_url: cloudResponseBackground?.url || sttProps.background_url || undefined,
